...
############Procrustes for 98 Brachynopus scutellaris individuals from 91 localities, maximum 25% missing data
...
setwd("~/Brachynopus/Procrustes final/all") ##full dataset
...
getstr<-read.structure('98ind_25miss_1327loc.stru', n.ind=98, n.loc=1327, onerowperind=T,col.lab=1,col.pop=2,row.marknames=0,  ask=F, NA.char=0)

#getstr<-read.structure('east_25miss_12216loci.stru', n.ind=25, n.loc=12216, onerowperind=T,col.lab=1,col.pop=2,row.marknames=0,  ask=F, NA.char=0)
#getstr<-read.structure('west_25miss_10616loci.stru', n.ind=51, n.loc=10616, onerowperind=T,col.lab=1,col.pop=2,row.marknames=0,  ask=F, NA.char=0)
#getstr<-read.structure('northSI_25miss_6755loci.stru', n.ind=18, n.loc=6755, onerowperind=T,col.lab=1,col.pop=2,row.marknames=0,  ask=F, NA.char=0)
...
dev.print(pdf, 'brachyall_genomicpca.pdf')
...
locs<-read.csv("brachyinds.csv")
#locs<-read.csv('eastinds.csv')
#locs<-read.csv('westinds.csv')
#locs<-read.csv('northSIinds.csv')
...
#Read in locality info and map color code (HEX codes) for the 91 localities
#table columns are popn, x, y, pca.color, loc.color

colorTable <- read.csv("brachypops.csv")
#colorTable <- read.csv('eastpops.csv')
#colorTable <- read.csv('westpops.csv')
#colorTable <- read.csv('northSIpops.csv')
...
par(mar=c(3,3,1,0));
par(pty='s');
plot.window(xlim=c(165, 180), ylim=c(-48, -33));
axis(1, at=seq(165, 180, by=5),  cex.axis=1);
axis(2, at=seq(-48, -33, by=5),  cex.axis=1,las=3);
#axis labels
...
dev.print(pdf, 'Brachy_latlong_98inds.pdf')
...
nzcoast <- raster(x='~/Brachynopus/Procrustes final/nz_coastline.asc')
...
dev.print(pdf, 'Brachy_map_98inds.pdf') #will not overwrite, must delete if rerunning
#dev.print(pdf, 'Brachy_map_zoomtoSI.pdf')
...
#max(resid_distances)
hist(geo_resid_distances/1000, breaks=50, main=paste("Brachy resids (km)"))
#hist(geo_resid_distances/1000, breaks=10)#for northern SI
hist(geo_resid_distances/1000, breaks=20,main=paste("Brachy resids (km)"))


hist(geo_resid_distances/1000, breaks=50,xlim=c(0,800), ylim=c(0,20), main=paste("Brachy resids (km)"))
dev.print(pdf, 'Brachy_geodist_resid_hist_98inds.pdf')
...
setwd("~/Brachynopus/Procrustes final/all")
...
#dev.print(pdf, 'Gl_env_latlong_dotsandarrows.pdf')
